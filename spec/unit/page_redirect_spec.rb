...
describe RadiantRedirectExtension::PageRedirect do
  ...
  context 'when the site controller is extended' do
    ...
    context 'when the redirect_page_layout_name is set' do
      before do
        SiteController.redirect_page_layout_name = 'test_redirect_page_layout_name'
      end

      its(:redirect_page_layout_name) { should == 'test_redirect_page_layout_name' }
      ...
      context 'when the redirect_url_field_name is set' do
        ...
        context 'when processing a page' do
          ...
          let(:fields) { [] }
          let(:layout_name) { '' }
          let(:layout) { double('Layout', :name => layout_name) }
          let(:page) { double('Page', :fields => fields, :layout => layout) }

          context 'when the page layout is not the redirect layout' do
            let(:layout_name) { 'some_layout' }
            it { should_not have_received(:redirect_to) }
          end

          context 'when the page layout is the redirect layout' do
            let(:layout_name) { 'test_redirect_page_layout_name' }
